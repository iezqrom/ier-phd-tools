...
class TherCam(object):
    ...
    def saveThermalFilm(self, output):
        ...
        writer = cv2.VideoWriter(
            "{}.avi".format(output), fourcc, 9, (frame_width, frame_height), True
        )
    ...
    def targetTempAuto(
        ...
        try:
            while True:
                ...
                # Dynamic ROI
                subdataK = dataK[
                    edge : edge + (120 - edge * 2), edge : edge + (160 - edge * 2)
                ]
                ...
                if eud < 20:
                    mask = (xs[np.newaxis, :] - indyD[0]) ** 2 + (
                        ys[:, np.newaxis] - indxD[0]
                    ) ** 2 < r ** 2
                    roiC = dataC[mask]
                    globals.temp = round(np.mean(roiC), 2)
                    sROI = 1
                ...
                if self.shutter_open_time and touched and end and shutter_closed:
                    if shutter_closed > touch_time_out and shutter_closed < (
                        touch_time_out + 0.2
                    ):
                ...
                if (
                    globals.temp > target_temp
                    and momen > pre_shutter_time_in
                    and momen < (pre_shutter_time_in + 0.2)
                    and not shutter_opened
                ):
        ...
        try:
            while True:
                ...
                if self.shutter_open_time and touched and end and shutter_closed_time:
                    if shutter_closed_time > touch_time_out and shutter_closed_time < (
                        touch_time_out + 0.2
                    ):
                        print("UNTOUCH CAMERA")
                        if event_touch:
    ...
    def openTimerStatic(self, output, centreROI, r=20, arduino=None, total_time_out=14):
        """
        Method function to measure temperature of ROI and trigger action when a given temperature is reached.
        The required parameters are output and target temperature.
        Globals: stimulus, timeout, centreROI
        ...
        try:
            while True:
                ...
                if momen > (total_time_out) and not end:
                    print("End time")
                    close_shutter_stamp = time.time()
                    end = True
                    shutter_opened = False

                if momen < pre_shutter_time_in:
                    mask = (xs[np.newaxis, :] - indy) ** 2 + (
                        ys[:, np.newaxis] - indx
                    ) ** 2 < r ** 2
                    roiC = dataC[mask]
                ...
                names = ["image", "shutter_pos", "fixed_ROI", "time_now", "sROI"]
                datas = [dataC, [globals.stimulus], [indx, indy], [momen], [sROI]]
                saveh5py(names, datas, tiff_frameLOCAL, f)
                tiff_frameLOCAL += 1
    ...
    def openTimer(
        self, output, centreROI, stimulus=1, r=20, arduino=None, total_time_out=10
    ):
        """
        Method function to measure temperature of ROI and trigger action when a given temperature is reached.
        The required parameters are output and target temperature.
    ...
    def saveShutterCam(self, output, cam):
        ...
        try:
            ...
            while True:
                ...
                coorF = np.repeat(
                    [globals.indx_saved, globals.indy_saved], len(data[0]) / 2
                )
    ...
    def saveShutter(self, output):
        ...
        try:
            ...
            while True:
                ...
                coorF = np.repeat(
                    [globals.centreROI["control"][0], globals.centreROI["control"][1]],
                    len(dataC[0]) / 2,
                )
                data_pp = np.append(data_p, [coorF], axis=0)

                mask = (xs[np.newaxis, :] - globals.centreROI["control"][1]) ** 2 + (
                    ys[:, np.newaxis] - globals.centreROI["control"][0]
                ) ** 2 < r ** 2
                roiC = dataC[mask]
                temp = round(np.mean(roiC), 2)
    ...
    def saveShutterPos(self, output):
        ...
        try:
            while True:
                ...
                mask = (xs[np.newaxis, :] - globals.centreROI[1]) ** 2 + (
                    ys[:, np.newaxis] - globals.centreROI[0]
                ) ** 2 < r ** 2
                roiC = dataC[mask]
                temp = round(np.mean(roiC), 2)
                print("Mean: " + str(round(np.mean(temp), 2)))
                ...
                f.create_dataset(
                    ("stimulus" + str(tiff_frameLOCAL)), data=[globals.stimulus]
                )
                f.create_dataset(
                    ("fixed_coor" + str(tiff_frameLOCAL)),
                    data=[globals.centreROI[0], globals.centreROI[1]],
                )
                f.create_dataset(
                    ("dynamic_coor" + str(tiff_frameLOCAL)), data=[indx[0], indy[0]]
                )
        ...
        try:
            ...
            while True:
                ...
                if eud < 20:
                    ...
                    mask = (xs[np.newaxis, :] - cROI[1]) ** 2 + (
                        ys[:, np.newaxis] - cROI[0]
                    ) ** 2 < r ** 2
                    roiC = dataC[mask]
                    globals.temp = round(np.mean(roiC), 2)
                    sROI = 0
                ...
                if (
                    keyboard.is_pressed("l") and shutter_opened
    ...
    def savePosMeanfixShuGains(self, output, r, event1=None):
        ...
        try:
            ...
            while True:
                ...
                f.create_dataset(
                    ("fixed_coor" + str(tiff_frameLOCAl)), data=[indx, indy]
                )
                f.create_dataset(
                    ("dynamic_coor" + str(tiff_frameLOCAl)), data=[indxD[0], indyD[0]]
                )
                f.create_dataset(
                    ("gains" + str(tiff_frameLOCAl)),
                    data=[globals.Kp, globals.Ki, globals.Kd],
                )
                f.create_dataset(
                    ("gain_outputs" + str(tiff_frameLOCAl)),
                    data=[globals.proportional, globals.integral, globals.derivative],
                )
    ...
    def plotLiveROINE(self, c_w="c", r=20, record="n", output=None):
        ...
        try:
            while True:
                ...
                if c_w == "c":
                    subdataK = dataK[
                        edgey : edgey + (120 - edgey * 2), edgexl : (160 - edgexr)
                    ]
                    subdataC = (subdataK - 27315) / 100
                    # print([subdataC <= 28.5])
    ...
    def plotLiveROINEcheck(self, c_w="c", r=20, record="n", output=None):
        ...
        try:
            while True:
                ...
                if (
                    not was_pressed
                    and keyboard.is_pressed("i")
                    and keyboard.is_pressed("up")
                ):
                    self.vmaxT += 1
    ...
    def LivePlotKernel(self, event1):
        ...
        img1 = ax1.imshow(
            dummy, interpolation="nearest", vmin=5, vmax=40, animated=True
        )
        img2 = ax1.imshow(
            dummy, interpolation="nearest", vmin=5, vmax=40, animated=True
        )
