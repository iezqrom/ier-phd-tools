...
class zaber(grabPorts):
    ...
    def __init__(self, n_device, winPort = None, port = None):
        self.ports = grabPorts()
        self.ports.zaberPort(winPort)
...
def read_reply(command):
        ...
        # try:
        #     self.sms_type = reply.message_type
        # except:
        #     self.sms_type = None
        #
        # try:
        #     self.device_add = reply.device_address
        # except:
        #     self.device_add = None
        #
        # try:
        #     self.ax_n = reply.axis_number
        # except:
        #     self.ax_n = None
        #
        # try:
        #     self.id_sms = reply.message_id
        # except:
        #     self.id_sms = None
        #
        # try:
        #     self.flag_reply = reply.reply_flag
        # except:
        #     self.flag_reply = None
        #
        # try:
        #     self.state_dev = reply.device_status
        # except:
        #     self.state_dev = None
        #
        # try:
        #     self.warn_flag = reply.warning_flag
        # except:
        #     self.warn_flag = None
        #
        # try:
...
# Manual control & UI for zaber

    # def manualConKeyboard(self, devices, amount):
    #     self.spotsPosX = {'C1': [], 'C2': [], 'NonC': []}
    #     self.spotsPosY = {'C1': [], 'C2': [], 'NonC': []}
    #
    #     print('\n Move COLTHER. \n Device 1 (x): left and right arrows \n Device 2 (y): up and down arrows \n Device 3 (z): "u" (up) and "d" (down) \n Press "o" to open the shutter \n Press esc or "c" to close shutter and end finding cold spots')
    #
    #     while True:
    #
    #         print('working')
    ...
    #             self.posX = devices[0].device.send("/get pos")
    #             self.posX = int(self.posX.data)
    #
    #             self.posY = devices[1].device.send("/get pos")
    #             self.posY = int(self.posY.data)
    #
    #             while True:
    #                 self.spotLabel = input('\n What label should we put to this spot? (C1 / C2 / NonC)     ')
    #                 if self.spotLabel in ('C1', 'C2', 'NonC'):
    #                     print('\n Please continue moving the COLTHER')
    #                     break
    #                 else:
    #                     print("\n Only 'C1', 'C2' and 'NonC' are valid answers")
    #                     continue
    #
    #             self.spotsPosX['{}'.format(self.spotLabel)].append(self.posX)
    #             self.spotsPosY['{}'.format(self.spotLabel)].append(self.posY)
    #
    #
    ...
    #             stdscr.clrtoeol()
    #
    #             # stdscr.addstr(10,0, 'Temperature: {}'.format(globals.temp))
    #             # stdscr.addstr(11,0, 'Distance: {}'.format(globals.distance))
    #
    #             stdscr.move(19, 59)
    #             stdscr.clrtoeol()
    #
    ...
    #             elif key == ord('a'): ### ZABER STEPS
    #                 while True:
    #                     amount = my_raw_input(stdscr, 20, 60, "\n How much would you like to move?").decode("utf-8")
    #                     try:
    #                         amount = int(amount)
    #                         stdscr.move(21, 0)
    #                         stdscr.clrtoeol()
    #                         break
    #                     except:
    #                         stdscr.addstr(23, 0, 'Only integers are valid numbers')
    #
    #
    #
    #             elif key == curses.KEY_LEFT:
    ...
    #
    #                 while True:
    #                     self.spotLabel = my_raw_input(stdscr, 20, 60, "\n What label should we put to this spot? (C1 / C2 / NonC)").decode("utf-8")
    #
    #                     if self.spotLabel == "C1":
    #                         break
    #                     elif self.spotLabel == "C2":
    #                         break
    #
    #                     elif self.spotLabel == "NonC":
    #                         break
    #                     else:
    #                         stdscr.addstr(30, 60," Only (C1 / C2 / NonC) are valid inputs")
    #
    #                     # self.spotLabel = input('\n What label should we put to this spot? (C1 / C2 / NonC)     ')
    #                     # if self.spotLabel in ('C1', 'C2', 'NonC'):
    #                     #     print('\n Please continue moving the COLTHER')
    ...
    #         curses.endwin()
    #
    #     # print(self.spotsPosX)
    #     # print(self.spotsPosY)
    #
    # def manualConKeyboard(self, devices, amount):
    #
    #     while True:
...
############# Another closed-loop
    # def maintainColdMeanROI2(self, amount, devices, set_point, range, event1, radio, arduino = None):
    #
    #     devices[0].device.move_abs(globals.posX)
    #     devices[1].device.move_abs(globals.posY)
    #     devices[2].device.move_abs(globals.posZ)
    ...
    #                     b = amount * 4 * diff
    #
    #                 if globals.temp > lowR and globals.temp < highR:
    #                     # print('here')
    #                     diff = abs(set_point - globals.temp)
    #                     a = amount * 4 * diff
    #                     b = amount * 10 * diff
...
# maintainCold3 WAS USED TO MAKE THE FIRST SUCCESSFUL CLOSED_LOOP
    # def maintainCold3(self, amount, devices, low_bound, high_bound, event1, arduino = None):
    #
    #     devices[0].device.move_abs(globals.posX)
    #     devices[1].device.move_abs(globals.posY)
    #     devices[2].device.move_abs(globals.posZ)
    ...
    #     previous_temp = globals.temp
    #
    #     set_point = (high_bound + low_bound)/2
    ...
    #                     devices[0].device.move_abs(0)
    #
    #                 if globals.temp < (set_point - 1.5): # BELOW lower bound
    #                     diff = abs(set_point - globals.temp)
    #                     a = amount * 10 * diff
    #                     b = amount * 4 * diff
    #
    #                 if globals.temp > (set_point + 1.5): # ABOVE upper bound
    #                     diff = abs(set_point - globals.temp)
    #                     a = amount * 10 * diff
    #                     b = amount * 4 * diff
    #
...
# def maintainCold(self, amount, devices, low_bound, high_bound, event1):
#
#     # previous_temp = 32
#
#     try:
...
#                 # print('Minimum temperature: ' + str(globals.temp))
#
#                 if globals.temp < low_bound:   #negative is up
#
#                     # start = time.time()
#                     p_temp = abs(globals.temp - low_bound)
#                     p_amount = round(3*amount*p_temp)
#                     print('Proportional amount UP: ' + str(p_amount))
#
#                     devices[2].device.move_rel(-int(p_amount))
#                     pos = devices[2].device.send("/get pos")
#                     pos = int(pos.data)
#                     previous_temp = globals.temp
#                     globals.pos = pos
...
#                     # print(end - start)
#
#                 elif globals.temp > high_bound:
#
#                     # start = time.time()
#                     p_temp = abs(globals.temp - high_bound)
#                     p_amount = round(amount*p_temp)
#                     print('Proportional amount DOWN: ' + str(p_amount))
#                     devices[2].device.move_rel(int(p_amount))  #positive is down
#                     pos = devices[2].device.send("/get pos")
#                     pos = int(pos.data)
#                     previous_temp = globals.temp
#                     globals.pos = pos
...
#                     # print(end - start)
#
#                 elif globals.temp < high_bound and globals.temp > low_bound:
#                     print('within range')
#
#                     # start = time.time()
#                     # p_temp = abs(globals.temp - high_bound)
...
#         sys.exit(0)
#
# def maintainCold2(self, amount, devices, low_bound, high_bound, event1, arduino = None):
#
#     devices[0].device.move_abs(globals.posX)
#     devices[1].device.move_abs(globals.posY)
#     devices[2].device.move_abs(globals.posZ)
...
#
#
#                 if globals.temp < low_bound:   #negative is up # if temperature goes below the lower bound zaber goes UP
#
#                     # start = time.time()
#                     p_temp = abs(globals.temp - low_bound)
#                     p_amount = round(10*amount*p_temp)
#                     # print('Proportional amount UP 1: ' + str(p_amount))
#
#                     devices[2].device.move_rel(-int(p_amount))
#
#                     if globals.temp < previous_temp:  # if the current temperature  is smaller than the previous temperature zaber goes UP even more
#
#                         diff_temp = abs(globals.temp - previous_temp)
#                         diff_amount = round(amount*diff_temp)
#
#                         # print('Proportional amount UP 2: ' + str(diff_amount))
#                         final_amount = abs(diff_amount + p_amount)
#                         print('UP-UP: ' + str(final_amount))
#
#                         devices[2].device.move_rel(-int(diff_amount))
#                         pos = devices[2].device.send("/get pos")
#                         pos = int(pos.data)
#                         previous_temp = globals.temp
#                         globals.posZ = pos
#
#                     elif globals.temp > previous_temp: # if the current temperature is bigger than the previous temperature zaber goes DOWN
#
#                         diff_temp = abs(globals.temp - previous_temp)
#                         diff_amount = round(amount*diff_temp)
#
#                         # print('Proportional amount DOWN 2: ' + str(diff_amount))
#                         final_amount = abs(diff_amount + p_amount)
#                         print('UP-DOWN: ' + str(final_amount))
#
#                         devices[2].device.move_rel(int(diff_amount))
#                         pos = devices[2].device.send("/get pos")
#                         pos = int(pos.data)
...
#                     # print(end - start)
#
#                 elif globals.temp > previous_temp: #positive is down # if temperature goes above the upper bound zaber goes DOWN
#
#                     # start = time.time()
#                     p_temp = abs(globals.temp - high_bound)
#                     p_amount = round(amount*p_temp)
...
#                     devices[2].device.move_rel(int(p_amount))
#
#                     if globals.temp < previous_temp: # if the current temperature is smaller than the previous temperature zaber goes UP
#
#                         diff_temp = abs(globals.temp - previous_temp)
#                         diff_amount = round(amount*diff_temp)
#
#                         # print('Proportional amount UP 2: ' + str(diff_amount))
#                         final_amount = abs(diff_amount + p_amount)
#                         print('DOWN-UP: ' + str(final_amount))
#
#                         devices[2].device.move_rel(-int(diff_amount))
#                         pos = devices[2].device.send("/get pos")
#                         pos = int(pos.data)
...
#                         # print('Proportional amount DOWN 2: ' + str(diff_amount))
#                         final_amount = abs(diff_amount + p_amount)
#                         print('DOWN-DOWN: ' + str(final_amount))
#
#                         devices[2].device.move_rel(int(diff_amount))
#                         pos = devices[2].device.send("/get pos")
#                         pos = int(pos.data)
...
#
#                         diff_temp = abs(globals.temp - previous_temp)
#                         diff_amount = round(2*amount*diff_temp)
#
#                         # print('Proportional amount UP 2: ' + str(diff_amount))
#                         # final_amount = abs(diff_amount - p_amount)
#
...
#                         globals.posZ = pos
#
#                     elif globals.temp > previous_temp: # TEMPERATURE IS GOING UP SO zaber goes DOWN
#
#                         diff_temp = abs(globals.temp - previous_temp)
#                         diff_amount = round(4*amount*diff_temp)
#
#                         # print('Proportional amount DOWN 2: ' + str(diff_amount))
#                         # final_amount = abs(diff_amount - p_amount)
#
...
#         sys.exit(0)
    ...
    # def zaber_test_2(low_bound, high_bound, step, devices):
    #
    #     start = time.time()
    #
    ...
    #         if globals.temp < low_bound:   #negative is up
    #             # print(devices)
    #             devices[0].device.move_rel(-step)
    #
    #             end = time.time()
    #             #
    #             globals.elapsed = end - start
    ...
    #         elif globals.temp > high_bound:
    #
    #             devices[0].device.move_rel(step)  #positive is down
    #
    #             end = time.time()
    #             #
    #             globals.elapsed = end - start
