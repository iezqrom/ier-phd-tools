...
class Zaber(grabPorts):
            ...
            while True:
                ...
                ### TERMINATE
                    ...
                    if not was_pressed and not touched['1'] and current_roi == '2':
                        ...
                        movetostartZabersConcu(devices, 'tactile', ['y'], grid['tactile']['1'])
                        movetostartZabersConcu(devices, 'tactile', ['x'], grid['tactile']['1'])
    ...
    def gridCon3pantiltScaling(self, devices, ardpantilt, platformcamera = None, arduino = None, default_pan_tilt_values = globals.PanTilts, grid = globals.grid, haxes = globals.haxes, rules = globals.rules):
        """
            (for dermatome distance experiment) Method for Object Zaber to move the 3 axes of THREE zabers with keyboard presses. Like a game!
            The coordinates of two positions can be saved with 'z' and 'x'
            This method was created and it is specific to the experiment in which we measure cold
                ...
                ### TERMINATE
                    ...
                    if not was_pressed:
                        if not touched:
                            ...
                            pre_touch = grid['tactile'][current_roi]['z'] - globals.touch_z_offset * globals.up_modifier_touch_height
                            movetostartZabersConcu(devices, 'tactile', ['z'], pos = pre_touch)
                            moveAxisTo(devices, 'tactile', 'y', grid['tactile'][current_roi]['y'])
                            moveAxisTo(devices, 'tactile', 'x', grid['tactile'][current_roi]['x'])
                            touching = grid['tactile'][current_roi]['z'] + round(globals.touch_z_offset * globals.down_modifier_touch_height)
