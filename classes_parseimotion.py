...
class ParseDirectos(object):
    ...
    def __init__(self, path, file, sep = ','):
        ...
        self.stringified_columns = [x.lower().replace(' ', '_') for x in list(self.readhead.columns)]
...
def chunkify(signal, chunk_duration, fps = 30):
    ...
    for i, chu in enumerate(range(0, len(signal), length_chunk)):
        try:
            ...
            lastin = np.pad(signal[chu:chu + length_chunk], (0, (length_chunk - len(signal[chu:chu + length_chunk]))))
            tofill[i, :] = lastin
...
def thresholdNmean(chunked_signal, threshold = 50):
    thresholded = np.where(chunked_signal > threshold, chunked_signal, 0 * chunked_signal)
