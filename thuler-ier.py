...
class Thuler(object):
    ...
    def interpolation(self, hor_threshold, ver_threshold):
        ...
        self.ar_hor_y_pixels[y_nans] = np.interp(
            Yx(y_nans), Yx(~y_nans), self.ar_hor_y_pixels[~y_nans]
        )
        self.ar_ver_x_pixels[x_nans] = np.interp(
            Xx(x_nans), Xx(~x_nans), self.ar_ver_x_pixels[~x_nans]
        )

        coef_Y = np.polyfit(
            np.arange(len(self.ar_hor_y_pixels)), self.ar_hor_y_pixels, 1
        )
        self.fn_hor_y = np.poly1d(coef_Y)

        coef_X = np.polyfit(
            np.arange(len(self.ar_ver_x_pixels)), self.ar_ver_x_pixels, 1
        )
